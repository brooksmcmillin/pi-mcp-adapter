...
describe("mcp-auth-flow explicit auth", () => {
  ...
  it("does not revoke another session's pending authorization when logging out", async () => {
    mocks.sdkAuth
      .mockImplementationOnce(async (provider) => {
        await provider.redirectToAuthorization(new URL("https://auth.example.com/session-a"));
        return "REDIRECT";
      })
      .mockImplementationOnce(async (provider) => {
        await provider.redirectToAuthorization(new URL("https://auth.example.com/session-b"));
        return "REDIRECT";
      })
      .mockImplementationOnce(async (provider) => {
        await provider.saveTokens({ access_token: "surviving-session", token_type: "Bearer" });
        return "AUTHORIZED";
      });
    const { completeAuthFromInput, createOAuthRuntime, hasPendingAuth, removeAuth, shutdownOAuth, startAuth } = await import("../mcp-auth-flow.ts");
    const { getAuthForUrl, getAuthStorageOptions } = await import("../mcp-auth.ts");
    const runtimeA = createOAuthRuntime();
    const runtimeB = createOAuthRuntime();
    const storageA = getAuthStorageOptions(undefined, process.cwd(), "session", "encrypted-file");
    const storageB = getAuthStorageOptions(undefined, process.cwd(), "session", "encrypted-file");
    const optionsA = { runtime: runtimeA, authStorageOptions: storageA };
    const optionsB = { runtime: runtimeB, authStorageOptions: storageB };
    const url = "https://api.example.com/mcp";
    try {
      await startAuth("session-logout", url, { url, auth: "oauth" }, optionsA);
      await startAuth("session-logout", url, { url, auth: "oauth" }, optionsB);
      await removeAuth("session-logout", optionsA);
      expect(hasPendingAuth("session-logout", storageA, runtimeA)).toBe(false);
      expect(hasPendingAuth("session-logout", storageB, runtimeB)).toBe(true);
      await completeAuthFromInput("session-logout", "code-b", optionsB);
      expect(getAuthForUrl("session-logout", url, storageB)?.tokens?.accessToken).toBe("surviving-session");
      expect(getAuthForUrl("session-logout", url, storageA)).toBeUndefined();
    } finally {
      await shutdownOAuth(runtimeA);
      await shutdownOAuth(runtimeB);
    }
