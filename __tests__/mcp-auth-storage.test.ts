...
describe("encrypted OAuth credential file store", () => {
  ...
  beforeEach(() => {
    ...
    options = getAuthStorageOptions(undefined, process.cwd(), undefined, "encrypted-file");
    resetTestAuthSecretStore();
  });
  ...
  it("selects only the explicit backend and ignores legacy path configuration", () => {
    ...
    expect(getAuthStorageOptions(123, process.cwd(), undefined, "encrypted-file")).toEqual({ credentialStore: "encrypted-file" });
    expect(() => getAuthStorageOptions(undefined, process.cwd(), undefined, "plaintext")).toThrow(/oauthCredentialStore/);
  });

  it("gives session persistence precedence without requiring a file key or touching either persistent store", () => {
    delete process.env.PI_MCP_ADAPTER_OAUTH_FILE_KEY;
    const session = getAuthStorageOptions(undefined, process.cwd(), "session", "encrypted-file");
    expect(session.credentialStore).toBeUndefined();
    saveAuthEntry("session-wins", { tokens: { accessToken: "session-secret" } }, "https://example.com/mcp", session);
    expect(getAuthForUrl("session-wins", "https://example.com/mcp", session)?.tokens?.accessToken).toBe("session-secret");
    expect(existsSync(join(agentDir, "mcp-oauth-encrypted"))).toBe(false);
    removeAuthEntry("session-wins", session);
    expect(getAuthEntry("session-wins", session)).toBeUndefined();
  });
