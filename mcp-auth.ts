...
function getOAuthLifecycleRecord(serverName: string, options: AuthStorageOptions): OAuthLifecycleRecord {
  const scope = options.persistence === 'session'
    ? getAuthStorageIdentity(options)
    : options.credentialStore ?? 'os';
  const key = JSON.stringify([serverName, scope]);
  let record = oauthLifecycleRecords.get(key);
  if (!record) {
    record = { generation: {}, revocations: 0 };
    oauthLifecycleRecords.set(key, record);
  }
  return record;
}

/** Capture immutable process-local authority for one server's OAuth lifecycle. */
export function captureOAuthAuthority(serverName: string, assertNow = true, options: AuthStorageOptions = {}): OAuthAuthority {
  const record = getOAuthLifecycleRecord(serverName, options);
...
export function beginOAuthRevocation(serverName: string, options: AuthStorageOptions = {}): () => void {
  const record = getOAuthLifecycleRecord(serverName, options);
  record.generation = {};
...
export function getAuthStorageOptions(
  ...
  if (persistence !== 'session' && oauthCredentialStore === 'encrypted-file') {
...
export function getAuthStorageIdentity(options: AuthStorageOptions = {}): string {
  return options.persistence === 'session'
    ...
    : options.credentialStore === 'encrypted-file'
      ? 'encrypted-file'
...
function authEntryCacheKey(
  ...
  if (options.persistence === 'session') return `${getAuthStorageIdentity(options)}\0${serverName}`;
...
function writeSecureAuthEntry(serverName: string, entry: AuthEntry, options?: AuthStorageOptions): void {
  try {
    ...
    if (options?.persistence === 'session' || options?.credentialStore === 'encrypted-file' || !shouldAttemptLinuxKeyringRecovery(error)) throw error;
...
function readAuthEntry(
  ...
  const cacheable = options?.persistence !== 'session' && behavior.migrateLegacy !== false && isAuthEntryCacheEnabled();
